...
help()
{
  echo "Usage: ./build.sh
    ...
    [ --hip=compiler ] Path to the HIP compiler (e.g., amdclang++, hipcc);
...
if [ -z "$sycl_compiler" ] && [ -z "$openmp_compiler" ] && [ -z "$kokkos_compiler" ] && [ -z "$cuda_compiler" ] && [ -z "$hip_compiler" ]; then
  echo "Error: At least one of --sycl, --openmp, --kokkos, --cuda, or --hip must be specified."
  help
  exit 1
fi
...
if [ -z "$arch" ] || { [ -z "$vendor" ] && [ -z "$cuda_compiler" ] && [ -z "$hip_compiler" ]; }; then
...
if [[ -n "$sycl_compiler" ]]; then
  ...
  (
    ...
    cmake --build "$SCRIPT_DIR/build/sycl/" -t label_propagation_sycl page_rank_sycl -j 8 > "$SCRIPT_DIR/build/cmake_sycl_build.log" 2>&1
  ) &
  pid=$!
  pids+=("$pid")
...
if [[ -n "$openmp_compiler" ]]; then
  ...
  (
    ...
    cmake --build "$SCRIPT_DIR/build/openmp/" -t label_propagation_openmp page_rank_openmp -j 8 > "$SCRIPT_DIR/build/cmake_openmp_build.log" 2>&1
  ) &
  pid=$!
  pids+=("$pid")
...
if [[ -n "$kokkos_compiler" ]]; then
  ...
  (
    ...
    cmake --build "$SCRIPT_DIR/build/kokkos/" -t label_propagation_kokkos page_rank_kokkos -j 8 > "$SCRIPT_DIR/build/cmake_kokkos_build.log" 2>&1
  ) &
  pid=$!
  pids+=("$pid")
...
if [[ -n "$cuda_compiler" ]]; then
  ...
  (
    ...
    cmake --build "$SCRIPT_DIR/build/cuda/" -t label_propagation_cuda page_rank_cuda -j 8 > "$SCRIPT_DIR/build/cmake_cuda_build.log" 2>&1
  ) &
  pid=$!
  pids+=("$pid")
...
# if hip compiler is specified
if [[ -n "$hip_compiler" ]]; then
  echo "Building with HIP compiler: $hip_compiler ..."
  (
    cmake -S $SCRIPT_DIR -B "$SCRIPT_DIR/build/hip/" -DCMAKE_HIP_COMPILER="$hip_compiler" -DCMAKE_BUILD_TYPE=Release -DBUILD_HIP=ON -DOFFLOAD_VENDOR="${vendor:-AMD}" -DOFFLOAD_TARGET="$arch" > "$SCRIPT_DIR/build/cmake_hip.log" 2>&1 && \
    cmake --build "$SCRIPT_DIR/build/hip/" -t label_propagation_hip page_rank_hip -j 8 > "$SCRIPT_DIR/build/cmake_hip_build.log" 2>&1
  ) &
  pid=$!
  pids+=("$pid")
  pid_name_map["$pid"]="HIP"
fi
