...
void warn_irrelevant_params(const cxxopts::ParseResult& res, GenKind kind, const std::function<void(const std::string&)>& warn) {
    ...
    for (const auto& k : all_gen_keys()) {
        if (res.count(k) && !relevant.count(k)) { warn(std::string("--") + k + " is ignored with --generator=" + get_spec(kind).name + "."); }
    }
}
...
bool validate_generator_params(GenKind kind, const Options& o, std::string& err) {
    switch (kind) {
        case GenKind::fixed:
            ...
            if (o.edge_size > o.vertices) {
                err = "--edge-size cannot exceed --vertices.";
                return false;
            }
            return true;
        ...
        case GenKind::hsbm:
            ...
            if (o.max_edge_size < o.min_edge_size) {
            ...
            if (kind == GenKind::planted || kind == GenKind::hsbm) {
                ...
                if (o.communities > o.vertices) {
                    err = "--communities cannot exceed --vertices.";
                    return false;
                }
                if (o.p_intra < 0.0 || o.p_intra > 1.0) {
                    err = "--p-intra must be within [0,1].";
                    return false;
                }
            }
            ...
            return true;
        default: err = "Unknown generator."; return false;
    }
}
} // anonymous namespace
...
Options parse_args(int argc, char** argv) {
    ...
    opts.add_options("Generator")("g,generator", "Generator: uniform|fixed|planted|hsbm", cxxopts::value<std::string>(out.generator))(
        ...
        "max-edge-size", "Max edge size (uniform/planted)", cxxopts::value<std::size_t>(out.max_edge_size))("edge-size", "Fixed edge size (fixed)", cxxopts::value<std::size_t>(out.edge_size))(
        "communities", "Number of communities (planted/hsbm)", cxxopts::value<std::size_t>(out.communities))(
        "p-intra", "Intra-community probability (planted/hsbm)", cxxopts::value<double>(out.p_intra))("p-inter", "Inter-community probability (hsbm)", cxxopts::value<double>(out.p_inter))(
        "seed", "Graph RNG seed (0=nondet)", cxxopts::value<unsigned int>(out.seed));

    // Group: Labels
    opts.add_options("Labels")("label-classes", "Generate random labels in [0,C) (0=skip)", cxxopts::value<std::size_t>(out.label_classes)->default_value("0"))(
        "label-seed", "Label RNG seed (0=nondet)", cxxopts::value<unsigned int>(out.label_seed));

    opts.add_options("Device")("threads", "GPU device threads (OpenMP; 0=auto)", cxxopts::value<size_t>(out.device.threads))(
    ...
    opts.add_options("Misc")("h,help", "Show help", cxxopts::value<bool>(out.help))("version", "Show version", cxxopts::value<bool>(show_version)->default_value("false"));
    ...
    if (gen_flags == 1) {
        if (flag_uniform)
            out.generator = "uniform";
        else if (flag_fixed)
            out.generator = "fixed";
        else if (flag_planted)
            out.generator = "planted";
        else
            out.generator = "hsbm";
    }
    ...
    if (!out.load_file.empty()) {
        ...
        if (result.count("generator") || flag_uniform || flag_fixed || flag_planted || flag_hsbm) { warn("--load specified: generator selection and parameters are ignored."); }
        for (const auto& k : all_gen_keys()) {
...
std::unique_ptr<Hypergraph> make_hypergraph(const Options& opts) {
    ...
    if (!opts.load_file.empty()) {
        ...
        if (opts.generator == "uniform") {
            hg = generate_uniform(opts.vertices, opts.edges, opts.min_edge_size, opts.max_edge_size, opts.seed);
        } else if (opts.generator == "fixed") {
            hg = generate_fixed_edge_size(opts.vertices, opts.edges, opts.edge_size, opts.seed);
        } else if (opts.generator == "planted") {
            hg = generate_planted_partition(opts.vertices, opts.edges, opts.communities, opts.p_intra, opts.min_edge_size, opts.max_edge_size, opts.seed);
        } else if (opts.generator == "hsbm") {
            hg = generate_hsbm(opts.vertices, opts.edges, opts.communities, opts.p_intra, opts.p_inter, opts.min_edge_size, opts.max_edge_size, opts.seed);
        } else {
    ...
    if (!opts.save_file.empty()) { hg->save_to_file(opts.save_file); }
...
void print_cli_summary(const Options& opts) {
    ...
    std::cout << "  Max iterations: " << opts.iterations << std::endl;
    std::cout << "  Tolerance: " << opts.tolerance << std::endl;
    if (!opts.load_file.empty()) {
        std::cout << "  Input: " << opts.load_file << std::endl;
    } else {
        std::cout << "  Vertices: " << opts.vertices << std::endl;
        std::cout << "  Edges: " << opts.edges << std::endl;
    ...
    if (opts.label_classes > 0) {
        std::cout << "  Labels: classes=" << opts.label_classes << ", seed=" << opts.label_seed << std::endl;
    } else {
        std::cout << "  Labels: skipping. Using default labels if present.\n";
    }

    if (!opts.save_file.empty()) { std::cout << "  Output: " << opts.save_file << std::endl; }
