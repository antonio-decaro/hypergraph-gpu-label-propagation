...
/**
 * @brief HIP implementation of hypergraph label propagation
 */
class LabelPropagationHIP : public LabelPropagationAlgorithm {
  public:
    explicit LabelPropagationHIP(const CLI::DeviceOptions& device);
    ~LabelPropagationHIP();

    PerformanceMeasurer run(Hypergraph& hypergraph, int max_iterations = 100, double tolerance = 1e-6) override;

    std::string get_name() const override { return "HIP"; }
  ...
  private:
    ...
    bool run_iteration_hip(const DeviceFlatHypergraph& flat_hg,
